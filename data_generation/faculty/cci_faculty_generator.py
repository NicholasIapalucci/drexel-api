...
def generate_cci_faculty_data(drexel_json):
    ...
    for row in cci_faculty_html.find_all("div", class_="row")[1:]:
        ...
        # Email
        email = find(lambda elem: elem["href"].startswith("mailto:"), row.find_all("div", class_="cell")[2].find_all("a")).decode_contents().strip()

        # Phone Number
        ps = row.find_all("div", class_="cell")[2].find_all("p")
        is_phone_number = lambda p: bool(regex.match(r"^\d{3}\.\d{3}\.\d{4}$", p.decode_contents().strip()))
    ...
    find(lambda college: college["name"] == "College of Computing and Informatics", drexel_json["colleges"])["faculty"] = cci_faculty